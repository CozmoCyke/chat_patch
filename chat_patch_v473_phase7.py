...
def iter_search_files():
    ...
    skip_dirs = {
        ".git", ".svn", "__pycache__", "dist", "build", ".idea", ".vscode",".chat_patch"
    }
...
# Catégorie 1b : err_handler.SyntaxError(..., Translate("clé non canonique"))
    ...
    if m and normalize_aggressive(m.group(2)) == target_norm:
        ...
        if m.group(2) != key:
            new_line = line[:m.start()] + m.group(1) + escape_cpp_string(key) + m.group(3) + line[m.end():]
    ...
    if m and same(m.group(2)) and 'LocalizationManager::Instance().Translate(' not in line:
        key, new_line = canon_translate_replacement(line[:m.start()] + m.group(1), m.group(2), line[m.end(3):])
...
def cmd_pseint_upsert(query: str) -> int:
    ensure_dirs()
    clear_preview_dir()
...
def cmd_lang_check_key(query: str) -> int:
    ...
    if cpp_hits:
        for file, lineno, line, cat in cpp_hits[:20]:
            ...
            mkey = re.search(r'LocalizationManager::Instance\(\)\.Translate\("([^"]*)"\)', line)
            if mkey:
    ...
    for lang in sorted(LANG_FILES):
        ...
        if state == "missing":
            print(f'       suggestion: chat_patch lang-upsert {lang} "{canonical_key}" "<TRADUCTION>"')
