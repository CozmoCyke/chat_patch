...
def extract_strings_from_text(text: str, rel_file: str):
    ...
    for literal in iter_string_literals_with_lines(text):
        ...
        normalized = normalize_localization_key(raw_inner)
        if not normalized:
            continue
...
def preview_lang_upsert(path: Path, key: str, value: str):
    ...
    if exact_entries:
        if any(e["value"] == value for e in exact_entries):
            state = "exact-same"
        else:
            state = "exact-update"
    elif norm_entries:
...
def cmd_pseint_patcher(batchfile: str) -> int:
    ...
    for row in ready_rows:
        ...
        print(f"Entrée existante: {'oui' if lang_preview['exact_entries'] else 'non'}")
        if lang_preview["exact_entries"]:
            current_values = ", ".join(
                repr(entry["value"]) for entry in lang_preview["exact_entries"]
            )
