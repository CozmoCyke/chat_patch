...
def classify_string_block(lines: list[str], line_idx: int, source_literal: str):
    ...
    if re.search(r'_ZZ\s*\($', prefix):
        wrapper = "_ZZ"
    elif re.search(r'_Z\s*\($', prefix):
        wrapper = "_Z"

    next_line, next_col = skip_ws_and_comments(lines, line_idx, start + len(source_literal))
    concat = False
    multiline = False
    if next_line is not None and next_col is not None and next_line < len(lines):
        if next_line != line_idx:
            multiline = True
        if lines[next_line].startswith('"', next_col):
            concat = True
            multiline = multiline or next_line != line_idx
